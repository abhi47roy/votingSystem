...
function authenticate(aadharCard, password) {
    console.log("username and password ",aadharCard, password)
    var deferred = Q.defer();

    db.users.findOne({ username: aadharCard }, function (err, user) {
...
function create(userParam) {
    ...
    // validation
    db.users.findOne(
        { username: userParam.aadharCard },
        function (err, user) {
            if (err) deferred.reject(err.name + ': ' + err.message);

            if (user) {
                // username already exists
                deferred.reject('Username "' + userParam.aadharCard + '" is already taken');
            } else {
                createUser();
            }
        });
